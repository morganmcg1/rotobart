...
_CHECKPOINT_FOR_DOC = "facebook/RotoBART-base"
_CONFIG_FOR_DOC = "RotoBARTConfig"
_TOKENIZER_FOR_DOC = "RotoBARTTokenizer"


RotoBART_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.RotoBARTConfig`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.FlaxPreTrainedModel.from_pretrained` method to load the
            model weights.
"""

RotoBART_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`jnp.ndarray` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.

            Indices can be obtained using :class:`~transformers.RotoBARTTokenizer`. See
...
RotoBART_ENCODE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`jnp.ndarray` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
RotoBART_DECODE_INPUTS_DOCSTRING = r"""
    Args:
...
class FlaxRotoBARTAttention(nn.Module):
...
class FlaxRotoBARTEncoderLayer(nn.Module):
    config: RotoBARTConfig
...
class FlaxRotoBARTDecoderLayer(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.self_attn_layer_norm = nn.LayerNorm(dtype=self.dtype)
        self.encoder_attn = FlaxRotoBARTAttention(
            config=self.config,
            embed_dim=self.embed_dim,
            num_heads=self.config.decoder_attention_heads,
            dropout=self.config.attention_dropout,
    ...
    def __call__(
        ...
        hidden_states, self_attn_weights = self.self_attn(
            hidden_states=hidden_states, position_ids=position_ids, attention_mask=attention_mask, init_cache=init_cache
        )
        hidden_states = self.dropout_layer(hidden_states, deterministic=deterministic)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)
...
class FlaxRotoBARTClassificationHead(nn.Module):
...
class FlaxRotoBARTEncoder(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    ...
    def setup(self):
        ...
        # RotoBART is set up so that if padding_idx is specified then offset the embedding ids by 2
        ...
        self.layers = FlaxRotoBARTEncoderLayerCollection(self.config, self.dtype)
...
class FlaxRotoBARTDecoder(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    embed_tokens: Optional[nn.Embed] = None
    ...
    def setup(self):
        ...
        self.layers = FlaxRotoBARTDecoderLayerCollection(self.config, self.dtype)
        self.layernorm_embedding = nn.LayerNorm(dtype=self.dtype)
...
class FlaxRotoBARTModule(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    ...
    def setup(self):
        ...
        self.encoder = FlaxRotoBARTEncoder(self.config, dtype=self.dtype, embed_tokens=self.shared)
        self.decoder = FlaxRotoBARTDecoder(self.config, dtype=self.dtype, embed_tokens=self.shared)
...
class FlaxRotoBARTPreTrainedModel(FlaxPreTrainedModel):
    config_class = RotoBARTConfig
    ...
    def init_weights(self, rng: jax.random.PRNGKey, input_shape: Tuple) -> FrozenDict:
        ...
        # make sure initialization pass will work for FlaxRotoBARTForSequenceClassificationModule
    ...
    @add_start_docstrings(RotoBART_ENCODE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=FlaxBaseModelOutput, config_class=RotoBARTConfig)
    def encode(
        self,
        ...
        Example::
            ...
            >>> from transformers import RotoBARTTokenizer, FlaxRotoBARTForConditionalGeneration

            >>> model = FlaxRotoBARTForConditionalGeneration.from_pretrained('facebook/RotoBART-large-cnn')
            >>> tokenizer = RotoBARTTokenizer.from_pretrained('facebook/RotoBART-large-cnn')
    ...
    @add_start_docstrings(RotoBART_DECODE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=FlaxBaseModelOutputWithPastAndCrossAttentions, config_class=RotoBARTConfig)
    def decode(
        self,
        decoder_input_ids,
        encoder_outputs,
        ...
        # it can be changed by FlaxRotoBARTAttention module
        if past_key_values:
...
append_call_sample_docstring(
    FlaxRotoBARTModel, _TOKENIZER_FOR_DOC, _CHECKPOINT_FOR_DOC, FlaxSeq2SeqModelOutput, _CONFIG_FOR_DOC
)


class FlaxRotoBARTForConditionalGenerationModule(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32
    bias_init: Callable[..., jnp.ndarray] = jax.nn.initializers.zeros

    def setup(self):
        self.model = FlaxRotoBARTModule(config=self.config, dtype=self.dtype)
...
class FlaxRotoBARTForConditionalGeneration(FlaxRotoBARTPreTrainedModel):
    module_class = FlaxRotoBARTForConditionalGenerationModule
    dtype: jnp.dtype = jnp.float32

    @replace_return_docstrings(output_type=FlaxCausalLMOutputWithCrossAttentions, config_class=RotoBARTConfig)
    def decode(
        self,
        decoder_input_ids,
        encoder_outputs,
...
FLAX_RotoBART_CONDITIONAL_GENERATION_DOCSTRING = """
    ...
    Mask filling example::
        ...
        >>> from transformers import RotoBARTTokenizer, FlaxRotoBARTForConditionalGeneration
        >>> tokenizer = RotoBARTTokenizer.from_pretrained('facebook/RotoBART-large')
        >>> TXT = "My friends are <mask> but they eat too many carbs."

        >>> model = FlaxRotoBARTForConditionalGeneration.from_pretrained('facebook/RotoBART-large')
...
overwrite_call_docstring(
    FlaxRotoBARTForConditionalGeneration, RotoBART_INPUTS_DOCSTRING + FLAX_RotoBART_CONDITIONAL_GENERATION_DOCSTRING
)
append_replace_return_docstrings(
    FlaxRotoBARTForConditionalGeneration, output_type=FlaxSeq2SeqLMOutput, config_class=_CONFIG_FOR_DOC
)


class FlaxRotoBARTForSequenceClassificationModule(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        self.model = FlaxRotoBARTModule(config=self.config, dtype=self.dtype)
        self.classification_head = FlaxRotoBARTClassificationHead(
            config=self.config,
...
append_call_sample_docstring(
    FlaxRotoBARTForSequenceClassification,
    _TOKENIZER_FOR_DOC,
    _CHECKPOINT_FOR_DOC,
    FlaxSeq2SeqSequenceClassifierOutput,
    _CONFIG_FOR_DOC,
...
class FlaxRotoBARTForQuestionAnsweringModule(nn.Module):
    config: RotoBARTConfig
    dtype: jnp.dtype = jnp.float32
    num_labels = 2
...
append_call_sample_docstring(
    FlaxRotoBARTForQuestionAnswering,
    _TOKENIZER_FOR_DOC,
    _CHECKPOINT_FOR_DOC,
    FlaxSeq2SeqQuestionAnsweringModelOutput,
    _CONFIG_FOR_DOC,
